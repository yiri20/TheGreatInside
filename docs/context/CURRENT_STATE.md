...
- **Portrait coverage: 88/95** (55→60, Portrait Sourcing Batch 1;
  ...
  87→88, Portrait Completion Phase 2D-2 (Ibn Khaldun), 2026-08-28 —
  see branch table above for names/sources; every other release was
  replacement/delivery-only). **Portrait
  delivery: 88 local / 0 remote-Wikimedia / 7 no-portrait** (Zheng He
  non-eligible, 6 eligible) — every portrait in the roster is self-hosted
  ...
  three people resolved (Benjamin Banneker and Joan of Arc via
  historically-grounded later-depiction paths, Ibn Khaldun via
  editorial_nonlikeness) of the original 9: Bruce
  Lee, Muhammad Ali, Fela Kuti (Tier B) and Coco Chanel,
...
Verified on `main` at the 2026-08-28 Portrait Completion Phase 2D-2
release (commit `e5b3b1e`): `tsc --noEmit` clean · `vitest run`
670/670 · `next build --webpack` clean · `peopleDirectory.spec.ts` +
`person.visual.spec.ts` + `compare.visual.spec.ts` 109/109 · Playwright
**286/286** at `--workers=1`. Live production check for Ibn Khaldun
...
later-depiction tradition). Zheng He (Phase 2C-2) is
...
95/95, 5 people remaining: differentiated strategies per person rather
...
Muhammad Ali, Fela Kuti; Tier C: Coco Chanel, Umm Kulthum)
