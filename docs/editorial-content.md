...
that will go stale. For the current summary and tier breakdown, see
[`docs/checkpoints/editorial.md`](checkpoints/editorial.md). For the
full batch-by-batch selection rationale, QA corrections, and per-person
diversity notes from the pilot through Batch 6, see
[`docs/archive/session-history/editorial-batch-history.md`](archive/session-history/editorial-batch-history.md)
(historical — not needed for routine editorial-content work).
