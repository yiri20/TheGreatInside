 ...
 *
 * Fixed twelve-candidate intake, mechanically selected from existing
 ...
 * politics-heavy batch: writers, scientists, an explorer, an architect,
 * a philosopher, an actor/inventor, a musician, and civil-rights figures,
 * spanning ancient/medieval through contemporary and six world regions.
 * All twelve were `held` solely because of `eligibility_v2` (none for
 * evidence-integrity reasons) and each received a first-time row-by-row
 * evidence-approval audit, moving to `evidence_approved`. Six received a
 * RUBRIC_CORRECTION for a single-instance score that had drifted into the
 * 85+ band (Bob Marley risk_tolerance/persistence, Norman Borlaug
 * persistence/achievement_drive, Marie Tharp detail_orientation, Al-Biruni
 * curiosity) — see each candidate JSON's own provenance notes and
 * `docs/checkpoints/roster27-twelve-person-fast-batch.md`. No candidate
 * was rescued toward eligibility; `eligibility_v2` was not touched.
 *
 * Same architecture as `generateRoster26.ts`:
 ...
 *   default.
 *
 * Usage: corepack pnpm@10 exec tsx src/dev/roster1000/generateRoster27.ts
 */
...
const OUT_PATH = join(process.cwd(), "src/data/people/roster27.ts");

// Explicit literal allowlist — exactly the 12 fixed roster27 candidates.
// Never a blanket "every evidence_approved/qa_passed candidate" filter.
const ROSTER_27_SLUGS = new Set([
  "lu-xun",
  "amelia-earhart",
  "zaha-hadid",
  "jorge-luis-borges",
...
function loadRoster27(): Candidate[] {
  ...
  return all
    .filter((c) => (c.status === "evidence_approved" || c.status === "qa_passed") && ROSTER_27_SLUGS.has(c.slug))
...
function main() {
  const candidates = loadRoster27();

  if (candidates.length !== ROSTER_27_SLUGS.size) {
    console.error(`Expected ${ROSTER_27_SLUGS.size} promotable roster27 candidate(s), found ${candidates.length}. Aborting.`);
    process.exitCode = 1;
    return;
  }
