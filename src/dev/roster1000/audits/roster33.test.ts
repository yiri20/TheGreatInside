...
describe("Roster33: cross-target identity integrity", () => {
  it("no duplicate ids, slugs, or Wikidata QIDs across all 239 production people", () => {
  ...
  it("SEED_PEOPLE and PEOPLE_INDEX remain in agreement at 239 people, all 14 targets present in both", () => {
  ...
  it("Haruki Murakami (evidence_approved but portrait-held) was deliberately NOT promoted this cycle", () => {
    const candidate = loadCandidate("haruki-murakami");
    expect(candidate.status).toBe("evidence_approved");
    expect(candidate.portrait?.status).toBe("held");
    expect(SEED_PEOPLE.some((p) => p.slug === "haruki-murakami")).toBe(false);
  });
});
