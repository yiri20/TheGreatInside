...
const TARGETS = ["genghis-khan", "serena-williams", "oprah-winfrey"] as const;
...
describe.each(TARGETS)("Legacy integrity batch 5: %s", (slug) => {
...
describe("Legacy integrity batch 5: cross-target identity integrity", () => {
  ...
  it("SEED_PEOPLE and PEOPLE_INDEX remain in agreement (239 people as of Roster33, 2026-09-16 -- this batch itself added none; the count reflects later roster-expansion cycles)", () => {
    expect(SEED_PEOPLE).toHaveLength(239);
    expect(PEOPLE_INDEX).toHaveLength(239);
  });
});

describe("Legacy integrity batch 5: no eligibility rescue, no unrelated drift", () => {
  ...
  it("no other legacy person's isMatchEligible flipped as a side effect (spot-check against a representative sample, including the batch-1/2/3/4 remediated people)", () => {
...
describe("Legacy integrity batch 5: legacy scoring-lock baseline", () => {
  // Exact remaining count intentionally not hardcoded here -- a future
  // batch, if it happens, will legitimately shrink it further, the same
  // class of change this file's own tests made to batches 2-4's tests.
...
describe("Legacy integrity batch 5: historical audit snapshot supersession", () => {
  it("none of the three batch-5 targets were part of PR #35's frozen historical audit sample", () => {
    // Same discipline as batches 2-4's own regression tests: proves this
    // cycle correctly does NOT need to extend SUPERSEDED_AUDIT_SLUGS, and
    // guards against a future remediation of one of the frozen-sample
...
describe("Legacy integrity batch 5: product-selection policy stays out of the audit", () => {
  it("genghis-khan (political/military primary identity) was audited on identical evidentiary terms as the other two targets -- no special exclusion, no extra row penalty", () => {
    // This cycle's own explicit instruction: the newer zero-politics
    // cohort-selection policy governs which NEW candidates get selected,
    // not whether an existing legacy row survives evidence review. Confirm
    // genghis-khan went through the same publication/eligibility pipeline
    // as serena-williams/oprah-winfrey, not a different one.
    const genghis = production["genghis-khan"];
    expect(genghis.status).toBe("published");
    expect(genghis.isDirectoryVisible).toBe(true);
    expect(candidates["genghis-khan"].status).toBe("evidence_approved");
    expect(genghis.attributes.length).toBeGreaterThan(0);
  });
});
