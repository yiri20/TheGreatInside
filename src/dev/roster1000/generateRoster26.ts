 ...
 *
 * Fixed ten-candidate intake, mechanically selected from existing
 * `data-pipeline/candidates/*.json` only — no new candidate discovery, no
 * new behavioral research. Che Guevara was already `qa_passed`; the other
 * nine (Fidel Castro, Jawaharlal Nehru, Ho Chi Minh, Salvador Allende,
 * Corazon Aquino, Muhammad Ali Jinnah, Nawal El Saadawi, Puyi, King Hussein
 * of Jordan) were `held` solely because of `eligibility_v2` and each
 * received a first-time row-by-row evidence-approval audit and moved to
 * `evidence_approved`. Two received a RUBRIC_CORRECTION for a single-
 * instance score that had drifted into the 85+ band (Castro `persistence`
 * 85->80, Allende `risk_tolerance` 88->80) — see each candidate JSON's own
 * provenance notes and `docs/checkpoints/roster26-ten-person-fast-batch.md`.
 * No candidate was rescued toward eligibility; `eligibility_v2` was not
 * touched.
 *
 * Same architecture as `generateRoster25.ts`:
 ...
 *   default.
 *
 * Usage: corepack pnpm@10 exec tsx src/dev/roster1000/generateRoster26.ts
 */
...
const OUT_PATH = join(process.cwd(), "src/data/people/roster26.ts");

// Explicit literal allowlist — exactly the 10 fixed roster26 candidates.
// Never a blanket "every evidence_approved/qa_passed candidate" filter.
const ROSTER_26_SLUGS = new Set([
  "che-guevara",
  "fidel-castro",
  "jawaharlal-nehru",
  "ho-chi-minh",
...
function loadRoster26(): Candidate[] {
  ...
  return all
    .filter((c) => (c.status === "evidence_approved" || c.status === "qa_passed") && ROSTER_26_SLUGS.has(c.slug))
...
function main() {
  const candidates = loadRoster26();

  if (candidates.length !== ROSTER_26_SLUGS.size) {
    console.error(`Expected ${ROSTER_26_SLUGS.size} promotable roster26 candidate(s), found ${candidates.length}. Aborting.`);
    process.exitCode = 1;
    return;
  }
