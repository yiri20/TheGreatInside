/**
 * ROSTER 37 -- second fresh new-candidate cycle after the recent-cohort
 * publication-vs-matching architecture diagnostic (CONTINUE_EXPANSION_AS_IS
 * classification). Continues the roster24-36 new-candidate lane.
 *
 * 15 people promoted here, all freshly researched, all evidence_approved,
 * zero backlog reuse -- Naomi Uemura, Edmund Hillary, Anita Roddick, Simone
 * de Beauvoir, and Mimar Sinan were all re-checked and remain genuinely
 * blocked (see docs/checkpoints/roster37.md). Every one of the 15 frozen
 * candidates this cycle cleared both evidence and portrait review -- no
 * holds this cycle.
 *
 * Same architecture as `generateRoster36.ts`:
 ...
 *
 * Usage: corepack pnpm@10 exec tsx src/dev/roster1000/generateRoster37.ts
 */
...
const OUT_PATH = join(process.cwd(), "src/data/people/roster37.ts");

// Explicit literal allowlist -- the 15 Roster37 promotions, all freshly
// researched this cycle, zero holds.
const ROSTER_37_SLUGS = new Set([
...
function loadRoster37(): Candidate[] {
  ...
  return all
    .filter((c) => (c.status === "evidence_approved" || c.status === "qa_passed") && ROSTER_37_SLUGS.has(c.slug))
...
function main() {
  const candidates = loadRoster37();

  if (candidates.length !== ROSTER_37_SLUGS.size) {
    console.error(`Expected ${ROSTER_37_SLUGS.size} promotable roster37 candidate(s), found ${candidates.length}. Aborting.`);
    process.exitCode = 1;
    return;
  }
 ...
 * \`src/dev/roster1000/generateRoster37.ts\`, which calls
...
export const ROSTER_37: readonly Person[] = seeds.map(build);
`;
