...
const seeds: PersonSeed[] = [
  ...
  },
  {
    ...
    rows: {
      ...
      // ERROR_CORRECTION (post-PR26-review, 2026-09; factual wording only, no score/confidence/evidenceType change): pursued serious technical invention work as a famous film actress at a time when this combination was virtually unprecedented. The resulting frequency-hopping patent (US Patent No. 2,292,387) was granted in 1942, but the U.S. Navy declined to adopt the proposed system at the time, considering the implementation impractical -- corrected from an earlier draft that inaccurately described the patent as dismissed/rejected by a 'military patent office'; the patent was in fact granted by the (separate, civilian) US Patent Office, and it was the Navy's later non-adoption decision, not any rejection, that is actually documented.
